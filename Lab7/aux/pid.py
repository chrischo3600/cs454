...
def run_controller(kp, kd, setpoint, noise, filtered, world: World):
    ...
    def pd_controller(x, y, kp, kd, setpoint):
        ...
        nonlocal lastErrorX, lastErrorY 
        
        errorX = setpoint.x - x
        errorY = setpoint.y - y
        changeInErrorX = errorX - lastErrorX
        changeInErrorY = errorY - lastErrorY
        lastErrorX = errorX
        lastErrorY = errorY
        P = [kp * errorX, kp * errorY]
        dt = 0.01
        D = [kd * changeInErrorX / dt, kd * changeInErrorY / dt]
