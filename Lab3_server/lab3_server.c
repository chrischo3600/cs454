...
int main(int argc, char* argv[])
{
	...
	tcsetattr(ifd, TCSANOW, &tio); // Apply new settings 
	...
	while(1)
	{
		...
		for (i = 0; (i < MSG_BYTES_MSG-1) && ((ch = fgetc(stdin)) != EOF) && (ch != '\n'); i++) {
			str[i] = ch; // save the character in the array
		}
			
		str[i] = '\0'; // char array -> string
		...
		// compute crc
		uint16_t crc;
		crc = pc_crc16(strPointer, i);
		printf("crc: %x\n", crc);
		
		// initialize attempts
		int attempts = 0;

		uint8_t ack = MSG_NACK;
		
		
		// make a message string for the write 
		N = i;
		char msg[4+N];
		msg[0] = (char) 0;		// start bit
		msg[1] = (char)(crc >> 8);	//  crc
    		msg[2] = (char)(crc & 0xFF);	//  crc
		msg[3] = (char) N;		// message length
		
		// input the message char by char
		for (int i = 0; i < N; i++) {
        		msg[4 + i] = str[i];
    		}
		...
		while (!ack)
		{
			...
			// read doesn't block, so we wait until it returns a non-zero value
